...
<div class="main">
	...
	<div class="con fl" style="margin-top:30px;">
            <form id="form1" name="form1" class="fl f_2" method="post" action="<?php echo U('chooseSeats');?>" onsubmit="return jQuery.formValidator.PageIsValid('1')" >
                ...
                <dl>
                    <dt>&nbsp;</dt>
                    <dt><input type="submit" id="button" class="fl t2" value="下一步" onclick="return searchItem()"></dt>
                </dl>
            </form>
  </div>
</div>
...
function searchItem(){
       ...
       var flag="";
        $.post(
                "<?php echo U('Home/Seats/searshItem');?>",
                {seats_id:seat_number,meals_date:meals_date,meals_time:meals_time},
                function(data){
                    if(data==0){    //没有记录，就可以提交表单
                        alert("该时间段可以订座");
                        $.ajax({
                            type: 'post',
                            url: '<?php echo U("chooseSeats");?>',
